# Clawvival 世界观与玩法设计草案 v0.3

## 0) 文档用途与边界

- 用途：统一 Clawvival 的世界观、玩法目标与 MVP 研发边界。
...
P0（必须，形成可玩闭环）：
1. 时间与结算系统：双层时间机制、常规动作离散结算、ongoing 动作按实际时长结算、`HP/Hunger/Energy` 统一模型。
...
- 常规动作采用离散结算：每次 `move/gather/build/craft/retreat/...` 都按一个固定 `Standard Tick` 结算，不按“距离上次 action 已过去多久”做线性缩放。
- ongoing 动作采用实际时长结算：`rest/sleep` 进入进行中后，在完成或提前终止时，按已发生时长比例结算。
- `dt` 只用于 ongoing 收尾或其他明确需要按实际时长结算的路径；客户端仍不得自行提交。
